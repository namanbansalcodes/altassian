...
export default function ResponsiveTable({
  ...
  return (
    <div className={`relative ${className}`} role="region" aria-label={label}>
      {canScrollLeft && (
        <div
          className={`absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r ${from} to-transparent z-10 pointer-events-none`}
        />
      )}
      {canScrollRight && (
        <div
          className={`absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l ${from} to-transparent z-10 pointer-events-none`}
        />
      )}
      <div
        ref={scrollRef}
